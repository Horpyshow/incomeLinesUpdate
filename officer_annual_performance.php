...
class AnnualPerformanceAnalyzer {
    ...
    public function getAnnualPerformanceSummary($year) {
        ...
        usort($officers, function($a, $b) {
            return $b['annual']['overall_score'] <=> $a['annual']['overall_score'];
        });
    ...
    private function calculateAnnualPerformance($officer_id, $year) {
        ...
        return [
            'total_target' => $result['total_annual_target'] ?? 0,
            'total_achieved' => $result['total_annual_achieved'] ?? 0,
            'achievement_percentage' => $achievement_percentage,
            'overall_score' => $overall_score,
            'grade' => $grade,
            'total_assigned_lines' => $result['total_assigned_lines'] ?? 0,
            'avg_working_days' => $result['avg_working_days'] ?? 0,
            'total_transactions' => $result['total_transactions'] ?? 0,
            'variance' => ($result['total_annual_achieved'] ?? 0) - ($result['total_annual_target'] ?? 0)
        ];
    }
    ...
    private function calculateQuarterlyPerformance($officer_id, $year) {
        ...
        foreach ($quarters as $quarter => $months) {
            ...
            $quarterly_data[$quarter] = [
                'target' => $result['quarter_target'] ?? 0,
                'achieved' => $result['quarter_achieved'] ?? 0,
                'achievement_percentage' => $achievement_percentage,
                'score' => $this->calculatePerformanceScore($achievement_percentage),
                'grade' => $this->calculatePerformanceGrade($achievement_percentage),
                'assigned_lines' => $result['assigned_lines'] ?? 0,
                'avg_working_days' => $result['avg_working_days'] ?? 0
            ];
        }
    ...
    private function calculateHalfYearlyPerformance($officer_id, $year) {
        ...
        foreach ($halves as $half => $months) {
            ...
            $half_yearly_data[$half] = [
                'target' => $result['half_target'] ?? 0,
                'achieved' => $result['half_achieved'] ?? 0,
                'achievement_percentage' => $achievement_percentage,
                'score' => $this->calculatePerformanceScore($achievement_percentage),
                'grade' => $this->calculatePerformanceGrade($achievement_percentage),
                'assigned_lines' => $result['assigned_lines'] ?? 0,
    ...
    private function getMonthlyBreakdown($officer_id, $year) {
        ...
        for ($month = 1; $month <= 12; $month++) {
            ...
            $monthly_data[date('M', mktime(0, 0, 0, $month, 1))] = [
                'month' => $month,
                'month_name' => date('F', mktime(0, 0, 0, $month, 1)),
                'target' => $result['month_target'] ?? 0,
                'achieved' => $result['month_achieved'] ?? 0,
                'achievement_percentage' => $achievement_percentage,
                'score' => $this->calculatePerformanceScore($achievement_percentage),
                'grade' => $this->calculatePerformanceGrade($achievement_percentage),
                'assigned_lines' => $result['assigned_lines'] ?? 0
