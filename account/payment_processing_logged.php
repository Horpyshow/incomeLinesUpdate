...
if ( isset($_POST['btn_post_general']) ) {
	...
	if($amount_paid != $amt_remitted){
		$error = true;
		$amount_remitted_Error = "<h4><strong>ATTENTION:</strong> Transaction failed! The <strong>amount posted: &#8358 {$amount_paid}</strong> MUST be equal to &#8358 {$amt_remitted} !</h4>";
	}
...
if (!$error) {
	...
	$wc_income_line = $_POST['income_line'];
		...
		$cuu_query = "UPDATE unposted_transactions SET trans_id='$txref', receipt_no='$receipt_no', payment_status='posted', reposting_time='$now' WHERE (remit_id='$remit_id' AND id='$repost_id')";
		$cuu_result = mysqli_query($dbcon, $cuu_query);
...
if ($debit_query)
	{
		...
		window.location.href='../leasing/view_trans.php';
		</script>
		<?php
	}
	else
	{
		...
		window.location.href='post_logged_trans_others.php?repost_id=<?php echo $repost_id; ?>';
		</script>
		<?php
	}
}
