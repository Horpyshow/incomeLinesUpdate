<?php
	...
	if ($_POST['decline']) {
		...
		if($table_row_no == 0) {
			$query_acct = "SELECT * FROM account_general_transaction WHERE id =".$pid;
			$acct_table_set = mysqli_query($dbcon, $query_acct);
		}
		...
		$query="UPDATE account_general_transaction_new SET approving_acct_officer_id = '$fc_id', approving_acct_officer_name = '$fc_name', approval_status = 'Declined', approval_time = '$now', verification_status = '' WHERE id=".$pid;
		...
		if ($result) {
			...
			$response['message'] = 'Record Declined Successfully ...';
		} else {
			$response['status']  = 'error';
			$response['message'] = 'Unable to decline record ...';
		}
